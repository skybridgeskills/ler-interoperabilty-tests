<script lang="ts">
	 ...
	 * interlocutor the operator converses with. No initiation input — one action
	 * button starts the passes; each pass credential lands in the artifact list
	 * (copy/download via {@link PassArtifactCard}); the wallet then asks INSIDE
	 * its surface, via the base `prompt` slot, what the operator's verifier
	 * decided (Accepted / Rejected + optional reason, one confirm per pass — no
	 * editing past verdicts; reset restarts the run). Purely presentational: the
	 * page owns the run state and the bindable verdict/reason selections.
	 */
	let {
		state = 'idle',
		busy = false,
...
<TestWallet
	...
	prompt={currentPassNumber !== undefined ? prompt : undefined}
	onRun={onStart}
	{onReset}
/>

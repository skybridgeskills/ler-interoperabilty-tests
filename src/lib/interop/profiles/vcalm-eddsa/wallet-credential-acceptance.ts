...
/** Wallet (holder) × Credential Acceptance × VCALM-EdDSA. */
export const walletCredentialAcceptance = WorkflowChecklist({
	...
	steps: [
		...
		},
		{
			...
			summary:
				'Receive the `verifiablePresentationRequest`, ask the user for consent, and post a verifiablePresentation that proves control of the holder’s DID using an EdDSA proof.',
			requirements: [
				{
					level: 'MUST',
		...
		},
		{
			...
			summary:
				'Parse the issuer’s verifiablePresentation, verify the eddsa-rdfc-2022 signature, resolve the issuer DID, validate dates, and check the Bitstring Status List.',
			requirements: [
				{
					level: 'MUST',
					text: 'Verify eddsa-rdfc-2022 signatures on credentials received from issuers.'
				},
				{
					level: 'MUST',
					text: 'Resolve issuer DIDs (did:web / did:key) to a DID document with an Ed25519 verification method.'
				},
				{
					level: 'MUST',

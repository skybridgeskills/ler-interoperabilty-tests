...
export const verifierCredentialRequestAndVerification = WorkflowChecklist({
	...
	steps: [
		...
		},
		{
			...
			summary:
				'Validate the verifiablePresentation structure, the holder DIDAuthentication proof, then verify each credential’s eddsa-rdfc-2022 signature, schema, status, and issuer trust.',
			requirements: [
				{
					level: 'MUST',
				...
				{
					level: 'MUST',
					text: 'Resolve holder DIDs (did:key / did:web) and verify the presentation proof signature, creation date, challenge, and `expires`.'
				},
				{
					level: 'MUST',
					text: 'Validate VCDM 2.0 + Open Badges 3.0 schema, required fields, and credential expiration on each extracted credential.'
				},
				{ level: 'MUST', text: 'Support `eddsa-rdfc-2022` proofs.' },
				{
					level: 'MUST',

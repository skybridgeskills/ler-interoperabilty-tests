...
/** Narration: the run started (after `plan` returns), before the floor lights. */
 ...
 * verifier's response endpoint. `submitted` false is a neutral fail note — the
 * operator still reports their verifier's decision (or re-presents a fresh
 * request); it does not itself mean the credential was rejected on its merits.
 */
export function presentedActivity(
	...
	return {
		...
		...(submitted ? {} : { detail: 'The presentation was not accepted at the response endpoint.' })
	};
}
 ...
 * attestations, and the automated floor outcomes from `inspect`.
 */
 ...
 * step-1/TLS rows light immediately after `inspect` while acceptance and
 * response-endpoint rows stay pending until scoring.
 */
...
/**
 * Whether the next credential should default to reusing the previous request:
 * yes only when the previous present actually reached the endpoint. Verifiers
 * that one-time their requests leave `submitted` false, so the operator is
 * nudged to paste a fresh one.
 */
export function defaultReuseRequest(previousEvidence: PresentEvidence | undefined): boolean {
	return previousEvidence?.submitted === true;
}

/** The request string to present credential `index` with, given the reuse choice. */
export function requestForPresent(reuse: boolean, freshInput: string, lastRequest: string): string {
	return reuse ? lastRequest : freshInput;
}

/**
 * Replace one credential's present evidence in place (transport-retry: a stale
 * reused request is re-presented with a fresh one before the verdict is

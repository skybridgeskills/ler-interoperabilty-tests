...
function evidence(passId: string, submitted = true): PresentEvidence {
	return {
		passId,
		submitted,
		transportStatus: submitted ? 200 : 400,
...
describe('narration', () => {
	...
	it('narrates a submitted present neutrally (info) and a miss as warn', () => {
		expect(presentedActivity(plan.entries[0], 0, 4, true).status).toBe('info');
		const miss = presentedActivity(plan.entries[1], 1, 4, false);
		expect(miss.status).toBe('warn');
		expect(miss.detail).toMatch(/not accepted/i);
	});
...
describe('evidence retry', () => {
	it('replaces only the target credential evidence, leaving others intact', () => {
		...
		expect(before[1].submitted).toBe(false);
	});
});
...
describe('checklist derivation', () => {
	it('lights the floor rows before scoring and the full report after', () => {
		...
		expect(preScore['vcalm.verifier-interaction-endpoint'].status).toBe('pass');
		expect(preScore['vcalm.verifier-request-tls'].status).toBe('fail');
		...
		expect(postScore['vcalm.verifier-accepts-valid-credential'].status).toBe('pass');
	});

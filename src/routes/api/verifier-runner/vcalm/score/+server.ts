...
import { ScoreVcalmRequest } from './score-schemas.js';
...
/**
 * Score an attested vcalm verifier run and return the `VerifierRunnerReport`.
 * Merges the automated floor outcomes, the VALID credential's delivery (the only
 * transport result that scores — it lights `vcalm.verifier-exchange-endpoint`),
...
export const POST = async ({ request }: { request: Request }) => {
	...
	const parsed = ScoreVcalmRequest.schema.safeParse(raw);
	if (!parsed.success) {
		return errorResponse('Bad request shape.', parsed.error.message);
	}

	try {
		const report = verifierRunner.scoreVcalmRun(parsed.data);
		return json(report);
	} catch (e) {
		...
		logger.error({ err: message }, 'verifier-runner vcalm score failed unexpectedly');

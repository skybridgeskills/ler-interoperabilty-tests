...
async function callPost(body?: unknown): Promise<{ status: number; payload: unknown }> {
	...
	return runInContext(ctx, async () => {
		const request = new Request('http://localhost/api/verifier-runner/vcalm/plan', {
			method: 'POST',
...
describe('POST /api/verifier-runner/vcalm/plan', { timeout: 30_000 }, () => {
	it('returns a credential-less four-entry vcalm plan (empty body)', async () => {
		const { status, payload } = await callPost();
		expect(status).toBe(200);
		const plan = payload as VerifierRunPlan;
		expect(plan.profile).toBe('vcalm');

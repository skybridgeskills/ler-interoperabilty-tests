...
/**
 * Generate a credential-less vcalm verifier run plan: four shuffled entries
 * ("Credential 1".."Credential 4"), one per pass kind, with no credentials —
 * VCALM fixtures are generated server-side at present time, each against a fresh
 * single-use exchange. Stateless: the client holds the plan and posts each entry
 * back to the present endpoint, then the whole plan to the score endpoint.
 */
export const POST = async ({ request }: { request: Request }) => {
	...
	try {
		const plan = verifierRunner.planVcalmRun({ cryptosuite: parsed.data.cryptosuite });
		return json(plan);
	} catch (e) {
		const message = e instanceof Error ? e.message : String(e);
		logger.error({ err: message }, 'verifier-runner vcalm plan failed unexpectedly');

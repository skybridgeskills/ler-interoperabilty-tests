...
function redirectFrom(search: string): { status: number; location: string } {
	try {
		load({ url: new URL(`http://localhost/wallet/credential-presentation/vcalm${search}`) });
	} catch (thrown) {
...
describe('/wallet/credential-presentation/vcalm redirect', () => {
	it('permanently redirects to the scenario', () => {
		const { status, location } = redirectFrom('');
		expect(status).toBe(308);
		expect(location).toBe('/scenarios/vcalm-wallet-presentation');
	});
	...
	it('preserves attach params, so the runbook keeps working', () => {
		...
		expect(location).toBe(
			'/scenarios/vcalm-wallet-presentation?exchangeId=probe-1&workflow=verify'
		);
	});
});
